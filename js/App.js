...
app.controller('CtrlGlobo',function($scope,$http){
		...
		var tmr = setInterval(function(){
			if (bAvancar){
				bAvancar = false;
				   $http({
					  method: 'GET',
					  url: 'https://api.github.com/users/globocom/repos?page=' + pageProj + '&per_page=100'
				   }).then(function (data){
						  pageProj += 1;
						  bAvancar = false;
	...
	$scope.mostrarDetalhes = function(projeto,index){
		   ...
		   $http({
			  method: 'GET',
			  url: 'https://api.github.com/repos/globocom/' + projeto + '?'
		   }).then(function (data){
		   ...
		   $http({
			  method: 'GET',
			  url: 'https://api.github.com/repos/globocom/' +  projeto +'/commits?page=1&per_page=20'
		   }).then(function (data){
	...
	$scope.mostrarMais = function(projeto){
		 ...
		 $http({
			  method: 'GET',
			  url: 'https://api.github.com/repos/globocom/' +  projeto +'/commits?page=' + page +'&per_page=20'
		   }).then(function (data){
